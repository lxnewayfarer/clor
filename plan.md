...
## 11. Улучшенное визуальное логирование (real-time)

**Задача**: Пользователи хотят видеть, что делает каждый агент прямо сейчас — без ручного открытия лог-панели.

### Текущее состояние

- Стриминг stdout уже реализован через SSE (`/api/run/{id}/logs/{nodeId}/stream`)
- Лог-панель открывается вручную кнопкой "logs" снизу, показывает один узел
- Узел в canvas показывает только `{status} {elapsed}s — {message}`

### Архитектурные решения

#### Backend: `orchestrator.go`

Обновлять `NodeStatus.Message` последней непустой строкой вывода агента. Для этого в `executeNode()` подписаться на LogStream и обновлять Message + broadcast при каждом новом чанке. Это не требует новых HTTP endpoints.

**Реализация**: добавить горутину-наблюдатель в `executeNode()` — читает из LogStream, извлекает последнюю непустую строку, вызывает `orch.setStatus(nodeID, s)` и `orch.broadcast()`.

#### Frontend: `web/index.html`

1. **Инлайн-превью на узле**: добавить `<div class="node-live-preview">` в HTML узла. Открывать SSE-стрим для каждого running-узла автоматически (не только для выбранного). Показывать последние 1–2 строки вывода прямо на узле.

2. **Авто-открытие лог-панели**: при старте запуска автоматически открывать лог-панель и подключаться к первому running-узлу. При переходе следующего узла в `running` — авто-переключаться (если пользователь сам не выбрал другой).

3. **Временны́е метки в логах**: каждый SSE-чанк prefixить `[HH:MM:SS]` на стороне клиента перед добавлением в лог-вьювер.

4. **ANSI strip**: простой regex `\x1b\[[0-9;]*m` перед отображением.

### Файлы

| Файл | Изменения |
|------|-----------|
| `orchestrator.go` | Горутина-наблюдатель LogStream → обновлять `NodeStatus.Message` последней строкой вывода |
| `web/index.html` | Инлайн-превью на узлах, авто-открытие лог-панели, авто-переключение на running-узел, timestamps, ANSI strip |

### Subtasks

1. В `orchestrator.go` в функции `executeNode()` добавить горутину, которая подписывается на LogStream узла и при каждом новом чанке извлекает последнюю непустую строку, записывает её в `NodeStatus.Message` и вызывает `broadcast()`.

2. В `web/index.html` добавить `<div class="node-live-preview">` в шаблон узла и при переходе узла в статус `running` открывать EventSource на `/api/run/{id}/logs/{nodeId}/stream`, буферизовать последние 2 строки в JS-map по nodeId и обновлять `node-live-preview` на каждый чанк.

3. В `web/index.html` при старте запуска автоматически открывать лог-панель (`logsVisible = true`), подключать лог-вьювер к первому running-узлу, и при смене `running`-узла авто-переключаться, если пользователь вручную не выбрал другой.

4. В `web/index.html` в функции добавления чанка в лог-вьювер: стрипить ANSI escape-последовательности (`str.replace(/\x1b\[[0-9;]*m/g, '')`) и префиксить каждый чанк временно́й меткой `[HH:MM:SS]`.

**Приоритет**: Высокий — напрямую отвечает на запрос пользователя.
